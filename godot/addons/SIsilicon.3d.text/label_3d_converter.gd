...
func collect_contours(edges: Dictionary, contours: Array) -> void:
	...
	var contour := Contour.new(
		start_edge.direction == Vector2.UP || start_edge.direction == Vector2.RIGHT
	)
...
func distance_to_segment(point, a, b) -> float:
	...
	var plane := Plane(Vector3(a.x, a.y, 0), Vector3(b.x, b.y, 0), Vector3(a.x, a.y, 1))

	var projected := plane.project(Vector3(point.x, point.y, 0))
	var t := (
		inverse_lerp(a.x, b.x, projected.x)
		if a.x != b.x
		else inverse_lerp(a.y, b.y, projected.y)
	)
