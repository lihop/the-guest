...
enum CameraState { STATE_NORMAL, STATE_ZOOM }
...
func _ready():
	$CanvasLayer/PlayerUI/Crosshair.texture = preload("../../texture/player_ui/normal.png")
	_crosshair_textures = _load_crosshair_textures(
		"res://addons/thief_controller/texture/player_ui"
	)
...
func set_crosshair_state(new_state: String):
	if new_state == "hidden":
		$CanvasLayer/PlayerUI/Crosshair.visible = false
	else:
		$CanvasLayer/PlayerUI/Crosshair.visible = true
