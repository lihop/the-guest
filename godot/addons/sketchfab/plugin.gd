...
func get_plugin_icon():
	...
	return Utils.create_texture_from_file(
		"res://addons/sketchfab/icon.png.noimport", _get_editor_scale() / 2.0
	)
...
func _get_editor_scale():
	...
	if display_scale == 0:
		...
		return (
			2.0
			if OS.get_screen_dpi(screen) >= 192 && OS.get_screen_size(screen).x > 2000
			else 1.0
		)
		...
		return (
			settings.get("interface/editor/custom_display_scale")
